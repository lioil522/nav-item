...
- **导出**：把 `menus`、`sub_menus`、`cards`、`ads`、`friends`、`site_settings`、`users`（含密码 hash）打包成一个 JSON 文件下载到本地。
- **导入**：上传备份 JSON，系统会先**清空**上述表，再按备份内容重写；导入成功后自动登出，请用**备份时**的账号密码重新登录。
...
- **导入**：新实例登录后台 → 「数据备份」→ 选择备份 JSON → 「确认导入并覆盖现有数据」→ 提示自动登出，用**备份时**的账号密码重新登录即可。

备份内容：栏目、子栏目、卡片、广告、友链、站点设置、**用户账号（含密码 hash）**。
